...
uint8_t Ai_Application_Ispu_PnPL_vtblGetStatus(IPnPLComponent_t *_this, char **serializedJSON, uint32_t *size, uint8_t pretty)
{
  ...
  bool temp_b = 0;
  ai_application_ispu_get_enabled(&temp_b);
  json_object_dotset_boolean(JSON_Status, "ai_application_ispu.enabled", temp_b);
...
uint8_t Ai_Application_Ispu_PnPL_vtblExecuteFunction(IPnPLComponent_t *_this, char *serializedJSON)
{
  ...
  if (json_object_dothas_value(tempJSONObject, "ai_application_ispu*load_ucf.arguments"))
  {
    ...
    if (json_object_dothas_value(tempJSONObject, "ai_application_ispu*load_ucf.arguments.ucf_content_size"))
    {
      ucf_content_size =(int32_t) json_object_dotget_number(tempJSONObject, "ai_application_ispu*load_ucf.arguments.ucf_content_size");
      if (json_object_dothas_value(tempJSONObject, "ai_application_ispu*load_ucf.arguments.ucf_content"))
      {
        ucf_content = json_object_dotget_string(tempJSONObject, "ai_application_ispu*load_ucf.arguments.ucf_content");
        ai_application_ispu_load_ucf(p_if_owner->cmdIF, ucf_content_size, (char*) ucf_content);
      }
    }
  }
  json_value_free(tempJSON);
